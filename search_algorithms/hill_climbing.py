...
def run_hill_climb(blur_img, clear_img, step_size=0.1, max_iterations=500, targetedVariance_inp=600):
    # Test the hill climbing algorithm
    n = 3
    start_matrix = np.random.rand(n, n)
    global blurred_image
    global targetedVariance
    targetedVariance = targetedVariance_inp

    blurred_image = cv2.imread(blur_img, cv2.IMREAD_GRAYSCALE)
    sharp_image = cv2.imread(clear_img, cv2.IMREAD_GRAYSCALE)
